...
// Interface para histórico agrupado por xtreino
interface PlayerXtreinoHistory {
  date: string;
  xtreinoId: number;
  teamName: string;
  q1Kills: number;
...
export default function JogadoresTab() {
  ...
  // ===== HISTÓRICO AGRUPADO POR XTREINO =====
  // Agrupa os registros de playerXtreinoStats por (date + xtreinoId), somando kills
  const selectedPlayerXtreinoHistory = useMemo(() => {
    if (!playerDetail) return [];
    
    const allNicks = [
      playerDetail.nickname, 
      ...(playerDetail.previousNicks ?? [])
    ].map(n => n.trim().toLowerCase());

    // Filtra stats do jogador (por qualquer nick)
    const playerStats = playerXtreinoStats.filter((s) =>
      allNicks.includes(s.playerName.trim().toLowerCase())
    );

    // Agrupa por (date, xtreinoId) — junta registros da mesma queda
    const grouped = new Map<string, PlayerXtreinoHistory>();

    for (const stat of playerStats) {
      const key = `${stat.date}-${stat.xtreinoId}`;
      const existing = grouped.get(key);

      if (existing) {
        // Soma kills (caso tenha múltiplos registros do mesmo jogador na mesma queda)
        existing.q1Kills += stat.q1Kills;
        existing.q2Kills += stat.q2Kills;
        existing.q3Kills += stat.q3Kills;
        existing.totalKills += stat.totalKills;
        existing.killPoints += stat.killPoints;
        // Mantém o teamName do último registro (ou poderia concatenar)
        ...
        grouped.set(key, {
          ...
          q1Kills: stat.q1Kills,
          q2Kills: stat.q2Kills,
          q3Kills: stat.q3Kills,
    ...
    return (
      <div className="space-y-6">
        <div className="bg-[#12121a] rounded-xl border border-[#2a2a3a] p-6">
          ...
          {/* Histórico de XTreinos - AGRUPADO */}
                  ...
                  <tbody className="divide-y divide-[#2a2a3a]">
                    {selectedPlayerXtreinoHistory.map((stat) => (
                      <tr key={`${stat.date}-${stat.xtreinoId}`} className="hover:bg-[#1a1a24]">
                        <td className="px-4 py-2 text-sm text-[#f0f0f5]">
                          {stat.date.split("-")[2]}/{stat.date.split("-")[1]}/{stat.date.split("-")[0]}
                        </td>
          ...
          {selectedPlayerXtreinoHistory.length === 0 && (
            <div className="text-center py-8">
              ...
              {selectedMonth && (
                <p className="text-[#3a3a4e] text-xs mt-1">
                  Filtro aplicado: {selectedMonth.split("-")[1]}/{selectedMonth.split("-")[0]}
                </p>
              )}
            </div>
          )}
