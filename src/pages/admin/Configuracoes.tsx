  ...
  useEffect(() => {
    if (settings) {
      setForm({
        ...
        primaryColor: settings.primaryColor ?? "#ff3b3b",
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input value={form.orgName} onChange={(e) => setForm({ ...form, orgName: e.target.value })}
                  className="w-full px-3 py-2.5 rounded-lg bg-[#1a1a24] border border-[#2a2a3a] text-[#f0f0f5] text-sm focus:outline-none focus:border-red-500/50" />
              </div>
              <div>
          ...
          <div className="bg-[#12121a] rounded-xl border border-[#2a2a3a] p-6">
            ...
            <textarea value={form.whatsappTemplate} onChange={(e) => setForm({ ...form, whatsappTemplate: e.target.value })} rows={15}
              className="w-full px-3 py-2.5 rounded-lg bg-[#1a1a24] border border-[#2a2a3a] text-[#f0f0f5] text-sm font-mono focus:outline-none focus:border-red-500/50" />
          </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <input value={form.primaryColor} onChange={(e) => setForm({ ...form, primaryColor: e.target.value })}
                  className="flex-1 px-3 py-2.5 rounded-lg bg-[#1a1a24] border border-[#2a2a3a] text-[#f0f0f5] text-sm focus:outline-none focus:border-red-500/50" />
              </div>
            </div>
          </div>
          ...
          <button type="submit" disabled={update.isPending}
            className="flex items-center gap-2 px-6 py-3 rounded-xl bg-red-500 hover:bg-red-600 text-white font-semibold transition-all disabled:opacity-50">
